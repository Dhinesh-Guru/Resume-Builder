 ...
 * Uses a Two-Pass Auto-Fit Engine to achieve perfectly equal 15mm margins on all 4 sides (Top, Bottom, Left, Right).
 */
export function exportNativeVectorPdf(resumeData, filename = 'ATS_Resume.pdf', allowTwoPages = false) {
  const pdf = new jsPDF('p', 'mm', 'a4');
  const pdfWidth = 210;
  const pdfHeight = 297;
  const margin = 15; // 15mm Top, Left, Right margin
  const contentWidth = pdfWidth - (margin * 2); // 180mm content width
  // Target baseline for the last line of text on Page 1:
  // Baseline at 274mm + ~8pt font descenders (~2.5mm) = 281.5mm bottom text boundary -> 297 - 282 = 15mm bottom margin!
  const targetBottomBaseline = 274;
  ...
  const runRenderPass = (secGap, itemG, lineSpacing, isRealDraw) => {
    let y = margin; // Start at top margin (15mm)

    const checkPageBreak = (neededHeight) => {
      if (y + neededHeight > targetBottomBaseline + 2) {
        if (allowTwoPages && isRealDraw) {
          pdf.addPage();
          y = margin;
        }
      }
    ...
    if (resumeData.summary) {
      ...
      summaryLines.forEach(line => {
        ...
        if (isRealDraw) pdf.text(line, margin, y);
        y += lineSpacing;
      });
      y += secGap - 2;
    }
    ...
    if (hasExtra || resumeData.skills || resumeData.otherSkills) {
      ...
      const renderSkillRow = (labelStr, valueStr) => {
        ...
        valLines.forEach((vLine, vIdx) => {
          if (vIdx === 0) {
            if (isRealDraw) pdf.text(vLine, margin + labelWidth, y);
          } else {
            checkPageBreak(lineSpacing);
            if (isRealDraw) pdf.text(vLine, margin + 4, y);
          }
          y += lineSpacing;
        });
      };
      ...
      if (!hasExtra && resumeData.skills) {
        ...
        sLines.forEach(l => {
          checkPageBreak(lineSpacing);
          if (isRealDraw) pdf.text(l, margin, y);
          y += lineSpacing;
        });
      }
  ...
  const initialSecGap = 5.0;
  const initialItemGap = 3.0;
  const initialLineH = baseFs * 0.43;

  // Pass 1: Measure exact un-expanded content height
  ...
  if (!allowTwoPages) {
    if (estimatedY < targetBottomBaseline - 10) {
      // Content is shorter than full page: expand spacing smoothly to hit 15mm equal bottom margin (targetBottomBaseline = 274mm)
      const deficit = targetBottomBaseline - estimatedY;
      const numSecs = 5;
      const numItems = 7;
      const numLines = 30;

      finalSecGap = initialSecGap + Math.min(3.5, (deficit * 0.45) / numSecs);
      finalItemGap = initialItemGap + Math.min(2.0, (deficit * 0.35) / numItems);
      finalLineH = initialLineH + Math.min(0.5, (deficit * 0.20) / numLines);
    } else if (estimatedY > targetBottomBaseline) {
      // Content exceeds single page target: compress gaps slightly to fit cleanly within 15mm margins
      const overflow = estimatedY - targetBottomBaseline;
      finalSecGap = Math.max(3.5, initialSecGap - (overflow * 0.4) / 5);
      finalItemGap = Math.max(2.0, initialItemGap - (overflow * 0.3) / 7);
      finalLineH = Math.max(3.8, initialLineH - (overflow * 0.3) / 30);
    }
  }

  // Pass 2: Draw native vector PDF with final auto-fitted spacing
